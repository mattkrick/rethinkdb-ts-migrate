...
async function connectToDb(config) {
    ...
    if (!tables.includes(MIGRATION_TABLE_NAME)) {
        ...
        await rethinkdb_ts_1.r.table(MIGRATION_TABLE_NAME).indexWait().run();
    }
    await rethinkdb_ts_1.r.db(config.db).wait({ waitFor: 'ready_for_writes' }).run();
}
/*
...
async function create(name, root) {
    ...
    const filename = (0, moment_1.default)().format('YYYYMMDDHHmmss') + '-' + name + '.ts';
