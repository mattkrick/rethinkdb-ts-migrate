...
async function connectToDb(config) {
    ...
    if (!tables.includes(MIGRATION_TABLE_NAME)) {
        ...
        await rethinkdb_ts_1.r.table(MIGRATION_TABLE_NAME).indexCreate('timestamp').run();
    }
}
/*
...
async function getMigrationsExcept(completedMigration, root, numToApply) {
    ...
    console.log('apply migs', migrationsToApply, completedMigration);
...
async function up(params) {
    ...
    const completedMigrations = await rethinkdb_ts_1.r.table(MIGRATION_TABLE_NAME)
    ...
    for (let i = 0; i < migrations.length; i++) {
        ...
        await rethinkdb_ts_1.r.table(MIGRATION_TABLE_NAME).insert({ name, timestamp }).run();
    }
...
async function down(params) {
    ...
    const migrationsToRollBack = all ? completedMigrations.reverse() : [completedMigrations[completedMigrations.length - 1]];
    ...
    for (let i = 0; i < migrations.length; i++) {
        ...
        await rethinkdb_ts_1.r.table(MIGRATION_TABLE_NAME).get(migration.id).delete().run();
    }
    logInfo('Migration successful');
}
exports.down = down;
